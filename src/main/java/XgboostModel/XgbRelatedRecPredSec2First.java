 ...
 */
public class XgbRelatedRecPredSec2First {
    HashMap<String, String> mvItemsMp ;
    int[] featidx;
    Booster bst;

    public XgbRelatedRecPredSec2First(String mvItemFeatsPath, String featsIdxPath, String ModelPath)
    ...
    public static DMatrix getPredDMtrix(String mv1str, String mv2Str, int[] featsIdx) throws XGBoostError{
        ...
        while (subStk.hasMoreTokens()) {
            ...
            if(p2.getIdx() < featsIdx[1] - 2){    //－2:打分和时间
                if(mvMp.containsKey(p2.getIdx())){
                    //pairSet.add(new Pair2(p2.getIdx() + offset, 1.0));
                    mvMp.put(p2.getIdx() + offset, 1.0);
                    double tmpVal = mvMp.get(p2.getIdx());
                    mvMp.put(p2.getIdx(), tmpVal + p2.getScore());
                }else {
                    mvMp.put(p2.getIdx(), p2.getScore());
                }
            }else if (p2.getIdx() == featsIdx[1] - 1  //把 打分时间 也做交叉特征处理
        ...
        for(Map.Entry<Integer, Double> entry : mvMp.entrySet()){
    ...
    public static void main(String[] args)throws IOException, XGBoostError{
        ...
        XgbRelatedRecPredSec2First XgbRelatedPred = new XgbRelatedRecPredSec2First(movieFeatsPath, featuresIdxpath, modelPath);
