...
/**
 * Created by qiguo on 18/1/3.
 * 存在交叉的特征对应的特征值写入方式：
 * 按两部电影存在交叉的特征之和作为其交叉特征的特征值，
 */
public class XgbRelatedRecPredJoinPlus {
    HashMap<String, String> mvItemsMp ;
    int[] featidx;
    Booster bst;

    public XgbRelatedRecPredJoinPlus(String mvItemFeatsPath, String featsIdxPath, String ModelPath)
    ...
    public static DMatrix getPredDMtrix(String mv1str, String mv2Str, int[] featsIdx) throws XGBoostError{
        ...
        while (subStk.hasMoreTokens()) {
            ...
            if(p2.getIdx() < featsIdx[1] - 2       //打分和上映时间
                    && mvMp.containsKey(p2.getIdx())){ //&& entry.getKey() >= featIdx[0]
                BigDecimal bgd = new BigDecimal(p2.getScore() + mvMp.get(p2.getIdx()));
                double joinval = Double.parseDouble(bgd.setScale(2, BigDecimal.ROUND_HALF_UP).toString());
                pairSet.add(new Pair2(p2.getIdx() + offset, joinval));
    ...
    public static void main(String[] args)throws IOException, XGBoostError{
        ...
        XgbRelatedRecPredJoinPlus XgbRelatedPred = new XgbRelatedRecPredJoinPlus(movieFeatsPath, featuresIdxpath, modelPath);
