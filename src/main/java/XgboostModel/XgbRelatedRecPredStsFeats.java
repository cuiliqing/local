 ...
 */
public class XgbRelatedRecPredStsFeats {
    HashMap<String, String>mvItemsMp;
    HashMap<String, String>mvStsFeatsMp;
    int[] featidx;
    Booster bst;

    public XgbRelatedRecPredStsFeats(String mvItemFeatsPath, String featsIdxPath, String ModelPath, String mvStsPath)
            throws IOException, XGBoostError {
        this.featidx = getFeatsIndices(featsIdxPath);
        this.mvItemsMp = getMoviesMp(mvItemFeatsPath);  // 电影的静态特征map
        this.bst = XGBoost.loadModel(ModelPath);
        this.mvStsFeatsMp = getMvStsFeatsMp(mvStsPath);  //统计特征
    }

    public HashMap<String, String> getMvStsFeatsMp(String path)throws IOException{
        HashMap<String, String>mvStsFeatsMp = new HashMap<String, String>();
        BufferedReader bfr = new BufferedReader(new FileReader(path));
        String line = "";
        while ((line = bfr.readLine()) != null){
            int idx = line.indexOf("\t");
            String mvId = line.substring(0, idx);
            mvStsFeatsMp.put(mvId, line.substring(idx+1));
        }
        bfr.close();
        return mvStsFeatsMp;
    }
    ...
    public static DMatrix getPredDMtrix(String mv1str, String mv2Str, int[] featsIdx, String stsFeatStr) throws XGBoostError{

        int offset = featsIdx[1] - 1;
        int totlen = 2 * offset - 1; //第一部电影＋交叉特征的总长度
        int stsFeatsStart = 3 * offset;
        ...
        int k = 0;
        for(String v : stsFeatStr.split("\t")){
            pairSet.add(new Pair2(stsFeatsStart + k++, Double.parseDouble(v)));
        }
    ...
    public static void main(String[] args)throws IOException, XGBoostError{
        ...
        String mvStsFtPath = args[3];    // 统计特征文件路径
        ...
        XgbRelatedRecPredStsFeats XgbRelatedPred = new XgbRelatedRecPredStsFeats(movieFeatsPath, featuresIdxpath, modelPath, mvStsFtPath);
        HashMap<String, String> mvFeatMp = XgbRelatedPred.mvItemsMp;
        int[] featIdx = XgbRelatedPred.featidx;
        HashMap<String, String>mvStsFeatsMp = XgbRelatedPred.mvStsFeatsMp;
        ...
        while ((line = bfr.readLine()) != null){
            ...
            for(Map.Entry<String, String>entry : mvFeatMp.entrySet()){
                if(!entry.getKey().equals(mvid)){
                    if(mvStsFeatsMp.containsKey(entry.getKey())){
                        DMatrix pred = getPredDMtrix(mvFeats, entry.getValue(), featIdx, mvStsFeatsMp.get(entry.getKey()));
