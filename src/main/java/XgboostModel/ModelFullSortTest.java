 ...
 */
public class ModelFullSortTest {
    HashMap<String, String> movieFeatsMp ;
    int[] featidx;
    Booster bst;

    public ModelFullSortTest(String mvItemFeatsPath, String featsIdxPath, String ModelPath)
    ...
    public static void main(String[] args)throws IOException, XGBoostError{
        ...
        ModelFullSortTest XgbRelatedPred = new ModelFullSortTest(movieFeatsPath, featuresIdxpath, modelPath);
        ...
        while ((line = bfr.readLine()) != null){
            ...
            for (Map.Entry<String, String>entry : mvFeatMp.entrySet()){
                if(!entry.getKey().equals(mvid)){
                    ...
                    DMatrix pred = getPredDMtrix(mvFeatMp.get(mvid), entry.getValue(), featIdx);
                    float[][] predScore = XgbRelatedPred.bst.predict(pred, true);
                    BigDecimal bgd = new BigDecimal(predScore[0][0]);
                    double res = Double.parseDouble(bgd.setScale(4, BigDecimal.ROUND_HALF_UP).toString());
